...
- Advanced IP Management & Threat Detection (10,000+ IPs)
- IP Blockchain Integration for Immutable IP Tracking
- Real-time IP Monitoring with Alert System
...
TOTAL_LINES = 128387  # Placeholder for line count
...
# CONFIGURATION
# =====================
CONFIG_DIR = ".blue_swan"
CONFIG_FILE = os.path.join(CONFIG_DIR, "config.json")
SSH_CONFIG_FILE = os.path.join(CONFIG_DIR, "ssh_config.json")
DATABASE_FILE = os.path.join(CONFIG_DIR, "blue_swan.db")
LOG_FILE = os.path.join(CONFIG_DIR, "blue_swan.log")
...
BLOCKCHAIN_DIR = os.path.join(CONFIG_DIR, "blockchain")
...
directories = [
    ...
    DOMAIN_HOSTING_DIR, BLOCKCHAIN_DIR, IP_MONITOR_DIR
]
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - BLUE_SWAN - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler(LOG_FILE, encoding='utf-8'),
        logging.StreamHandler(sys.stdout)
    ]
)
logger = logging.getLogger("BlueSwan")
...
class DatabaseManager:
    ...
    def init_tables(self):
        tables = [
            ...
            """,
            """
            CREATE TABLE IF NOT EXISTS ip_monitor_alerts (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                timestamp DATETIME DEFAULT CURRENT_TIMESTAMP,
                ip_address TEXT NOT NULL,
    ...
    def _create_default_admin(self):
        try:
            import hashlib
            default_password = "blue_swan_2024"
    ...
    def update_ip_scan(self, ip: str, open_ports: List[int], closed_ports: List[int], threat_level: str):
        try:
            self.conn.execute(
                """UPDATE managed_ips 
                   SET last_scan = CURRENT_TIMESTAMP, 
                       ...
                       threat_level = ?
                   WHERE ip_address = ?""",
                (json.dumps(open_ports), json.dumps(closed_ports), threat_level, ip)
            )
            self.conn.commit()
        except Exception as e:
            print(f"Failed to update IP scan: {e}")
    
    def add_ip_alert(self, ip: str, alert_type: str, description: str, severity: str):
        try:
            self.conn.execute(
                """INSERT INTO ip_monitor_alerts (ip_address, alert_type, description, severity)
                   VALUES (?, ?, ?, ?)""",
                (ip, alert_type, description, severity)
            )
            self.conn.commit()
            
            # Update alert count
            self.conn.execute(
                "UPDATE managed_ips SET alert_count = alert_count + 1 WHERE ip_address = ?",
                (ip,)
            )
            self.conn.commit()
        except Exception as e:
            print(f"Failed to add IP alert: {e}")
    
    def get_ip_alerts(self, ip: str = None, limit: int = 100) -> List[Dict]:
        try:
            if ip:
                rows = self.conn.execute(
                    "SELECT * FROM ip_monitor_alerts WHERE ip_address = ? ORDER BY timestamp DESC LIMIT ?",
                    (ip, limit)
                )
            else:
                rows = self.conn.execute(
                    "SELECT * FROM ip_monitor_alerts ORDER BY timestamp DESC LIMIT ?",
                    (limit,)
                )
            return [dict(row) for row in rows]
        except:
    ...
    def add_blockchain_block(self, block: 'BlockchainBlock') -> bool:
        try:
            self.conn.execute(
                """INSERT INTO blockchain_blocks (index, timestamp, ip_address, data, previous_hash, hash)
                   VALUES (?, ?, ?, ?, ?, ?)""",
                (block.index, block.timestamp, block.ip_address, block.data, block.previous_hash, block.hash)
            )
            self.conn.commit()
            return True
        except Exception as e:
            print(f"Failed to add blockchain block: {e}")
            return False
    
    def get_blockchain_blocks(self, ip: str = None, limit: int = 100) -> List[Dict]:
        try:
            if ip:
                rows = self.conn.execute(
                    "SELECT * FROM blockchain_blocks WHERE ip_address = ? ORDER BY index DESC LIMIT ?",
                    (ip, limit)
                )
            else:
                rows = self.conn.execute(
                    "SELECT * FROM blockchain_blocks ORDER BY index DESC LIMIT ?",
                    (limit,)
                )
            return [dict(row) for row in rows]
        except:
    ...
    def get_statistics(self) -> Dict:
        ...
        try:
            ...
            stats['total_blockchain_blocks'] = self.conn.execute("SELECT COUNT(*) FROM blockchain_blocks").fetchone()[0]
            stats['total_ip_alerts'] = self.conn.execute("SELECT COUNT(*) FROM ip_monitor_alerts").fetchone()[0]
        except:
            pass
        return stats
...
class BlockchainEngine:
    ...
    def load_chain(self):
        ...
        blocks = self.db.get_blockchain_blocks(limit=10000)
    ...
    def create_genesis_block(self):
        ...
        genesis_block = BlockchainBlock(
            ...
            ip_address="0.0.0.0",
            data="Genesis Block - Blue Swan Blockchain",
            previous_hash="0",
            hash=""
        )
        genesis_block.hash = self.calculate_hash(genesis_block)
        self.chain.append(genesis_block)
        self.db.add_blockchain_block(genesis_block)
    
    def calculate_hash(self, block: BlockchainBlock) -> str:
        """Calculate SHA-256 hash of a block"""
        block_string = f"{block.index}{block.timestamp}{block.ip_address}{block.data}{block.previous_hash}"
    ...
    def mine_block(self, ip_address: str, data: str) -> BlockchainBlock:
        """Mine a new block for an IP address"""
        ...
        nonce = 0
        while True:
            test_hash = hashlib.sha256(f"{block.index}{block.timestamp}{block.ip_address}{block.data}{block.previous_hash}{nonce}".encode()).hexdigest()
            if test_hash.startswith('0' * self.difficulty):
                block.hash = test_hash
                break
            nonce += 1
        
        self.chain.append(block)
        self.db.add_blockchain_block(block)
    ...
    def get_block_by_ip(self, ip: str) -> List[Dict]:
        """Get all blocks for a specific IP"""
        blocks = self.db.get_blockchain_blocks(ip)
        return blocks
    ...
    def get_ip_blockchain_data(self, ip: str) -> Dict:
        """Get blockchain data for an IP"""
        blocks = self.get_block_by_ip(ip)
        if not blocks:
            return {
                'ip': ip,
...
# IP MONITOR ENGINE
# =====================
class IPMonitorEngine:
    def __init__(self, db: DatabaseManager, config: ConfigManager, blockchain: BlockchainEngine):
        self.db = db
        self.config = config
        self.blockchain = blockchain
        self.running = False
        self.monitored_ips = {}  # ip -> IPMonitor object
        self.alert_callbacks = []
        self.scan_thread = None
        self.max_ips = config.get('ip_monitor.max_ips', 10000)
        self.scan_interval = config.get('ip_monitor.scan_interval', 300)
        self.alert_on_scan = config.get('ip_monitor.alert_on_scan', True)
        self.alert_on_block = config.get('ip_monitor.alert_on_block', True)
        self.blockchain_enabled = config.get('ip_monitor.blockchain_enabled', True)
    ...
    def load_monitored_ips(self):
        ...
        for ip_data in ips:
            ...
            self.monitored_ips[ip] = IPMonitor(
                ip=ip,
                domain=ip_data.get('domain'),
                open_ports=json.loads(ip_data.get('open_ports', '[]')),
                closed_ports=json.loads(ip_data.get('closed_ports', '[]')),
                last_scan=ip_data.get('last_scan', 'Never'),
                threat_level=ip_data.get('threat_level', 'low'),
                alert_count=ip_data.get('alert_count', 0),
                is_blocked=ip_data.get('is_blocked', False),
                blockchain_hash=ip_data.get('blockchain_hash', '')
            )
    ...
    def start(self):
        ...
        print(f"{Colors.GREEN}✅ IP Monitor started - Monitoring {len(self.monitored_ips)} IPs{Colors.RESET}")
    ...
    def _scan_ip(self, ip: str):
        ...
        try:
            # Perform port scan
            open_ports, closed_ports = self._port_scan(ip)
            
            # Determine threat level
            threat_level = self._determine_threat_level(open_ports)
            
            # Update database
            self.db.update_ip_scan(ip, open_ports, closed_ports, threat_level)
            
            # Update monitored IP
            if ip in self.monitored_ips:
                self.monitored_ips[ip].open_ports = open_ports
                self.monitored_ips[ip].closed_ports = closed_ports
                self.monitored_ips[ip].last_scan = datetime.datetime.now().isoformat()
                self.monitored_ips[ip].threat_level = threat_level
            
            # Add to blockchain if enabled
            if self.blockchain_enabled:
                block_data = f"SCAN: {ip} - Open ports: {len(open_ports)}, Closed ports: {len(closed_ports)}"
                block = self.blockchain.mine_block(ip, block_data)
                self.db.conn.execute(
                    "UPDATE managed_ips SET blockchain_hash = ? WHERE ip_address = ?",
                    (block.hash, ip)
                )
                self.db.conn.commit()
            
            # Check for alerts
            self._check_alerts(ip, open_ports, threat_level)
            
            logger.info(f"Scanned IP {ip}: {len(open_ports)} open ports, {len(closed_ports)} closed ports, Threat: {threat_level}")
            
        except Exception as e:
            logger.error(f"IP scan error for {ip}: {e}")
    
    def _port_scan(self, ip: str) -> Tuple[List[int], List[int]]:
        """Perform port scan on IP"""
    ...
    def _determine_threat_level(self, open_ports: List[int]) -> str:
    ...
    def _check_alerts(self, ip: str, open_ports: List[int], threat_level: str):
        """Check for alerts based on scan results"""
        if not self.alert_on_scan:
            return
        ...
        if sensitive_found:
            ...
            self.db.add_ip_alert(ip, "sensitive_ports", description, "high")
            self._trigger_alert(ip, "sensitive_ports", description, "high")
        
        if threat_level in ["critical", "high"]:
            description = f"Threat level: {threat_level.upper()}"
            self.db.add_ip_alert(ip, "threat_level", description, threat_level)
            self._trigger_alert(ip, "threat_level", description, threat_level)
    
    def _trigger_alert(self, ip: str, alert_type: str, description: str, severity: str):
    ...
    def add_ip(self, ip: str, domain: str = None, notes: str = "") -> bool:
        """Add an IP to monitoring"""
        try:
            ipaddress.ip_address(ip)
            if len(self.monitored_ips) >= self.max_ips:
                return False
            
            if ip not in self.monitored_ips:
                success = self.db.add_managed_ip(ip, domain, "monitor", notes)
                if success:
                    self.monitored_ips[ip] = IPMonitor(
                        ip=ip,
                        domain=domain,
    ...
    def get_ip_status(self, ip: str) -> Optional[Dict]:
        """Get status of a monitored IP"""
        if ip in self.monitored_ips:
            ip_data = self.monitored_ips[ip]
            return {
                'ip': ip_data.ip,
                'domain': ip_data.domain,
                'open_ports': ip_data.open_ports,
                'closed_ports': ip_data.closed_ports,
                'last_scan': ip_data.last_scan,
                'threat_level': ip_data.threat_level,
                'alert_count': ip_data.alert_count,
                'is_blocked': ip_data.is_blocked,
                'blockchain_hash': ip_data.blockchain_hash
            }
        return None
    ...
    def get_all_ips(self) -> List[Dict]:
        ...
        return [
            ...
            for ip_data in self.monitored_ips.values()
        ]
    
    def get_ip_alerts(self, ip: str = None) -> List[Dict]:
        """Get alerts for an IP"""
        return self.db.get_ip_alerts(ip)
    ...
    def setup(self) -> bool:
        ...
        async def on_message(message):
            ...
            if message.content.startswith(self.config.get('prefix', '!')):
                ...
                embed = discord.Embed(title="🦢 BLUE SWAN Response", description=f"```{output}```",
                                     color=0xFF0000)
...
class TelegramBot:
    ...
    def _run(self):
        try:
            async def main():
                self.client = TelegramClient('blue_swan_session', 1, 'dummy')
...
# =====================
class WebDashboard:
    def __init__(self, handler, db: DatabaseManager, config: ConfigManager, ip_monitor: IPMonitorEngine):
        self.handler = handler
        self.db = db
        self.config = config
    ...
    def create_app(self):
        ...
        <head>
            ...
            <title>🦢 BLUE SWAN - Cybersecurity Dashboard</title>
            ...
            <style>
                ...
                * { margin: 0; padding: 0; box-sizing: border-box; }
                body {
                    ...
                    color: var(--white);
                    min-height: 100vh;
                    background: radial-gradient(ellipse at 50% 0%, #1a0000 0%, #0a0a0a 70%);
                }
                .header {
                ...
                .header h1 span { color: var(--red-primary); }
                .header p {
                    color: var(--gray);
                    font-size: 0.9em;
                    letter-spacing: 2px;
                ...
                .status-online { background: rgba(255, 0, 0, 0.15); color: var(--red-primary); }
                .status-offline { background: rgba(255, 0, 0, 0.05); color: var(--gray); }
                .severity-critical { background: rgba(255, 0, 0, 0.3); color: #ff0000; }
                .severity-high { background: rgba(255, 100, 0, 0.2); color: #ff6600; }
                .severity-medium { background: rgba(255, 200, 0, 0.15); color: #ffcc00; }
                .severity-low { background: rgba(255, 0, 0, 0.1); color: var(--red-primary); }
                ::-webkit-scrollbar {
                    width: 4px;
                }
                ...
                @keyframes glow {
                    ...
                    50% { box-shadow: 0 0 30px var(--red-glow); }
                ...
                .dot.open { background: #ff0000; box-shadow: 0 0 10px rgba(255,0,0,0.5); }
                .dot.closed { background: #444444; }
                .ip-item .port-badge {
                    background: var(--red-dark);
                    padding: 2px 10px;
                    border-radius: 4px;
        ...
        <body>
            ...
            <div class="container">
                <div class="stats-grid" id="stats">
                    ...
                    <div class="stat-card"><h3 id="statDomains">0</h3><p>DOMAINS</p></div>
                ...
                <div class="section">
                    ...
                    <div style="display:flex; gap:10px; flex-wrap:wrap;">
                        ...
                        <input type="text" id="command" class="command-input" placeholder="Enter command..." style="flex:1; min-width:200px;">
                        <button onclick="executeCommand()">EXECUTE</button>
                        <button onclick="refreshData()" style="background:var(--dark-bg);"><i class="fas fa-sync-alt"></i> Refresh</button>
                    </div>
                ...
                <div class="section">
                    ...
                    <div class="monitor-header">
                        <span class="ip-badge"><i class="far fa-clock"></i> Live monitoring</span>
                    </div>
                    <div id="ipListContainer" class="ip-list">
                ...
                <div class="chart-grid">
                    <div class="chart-card">
                        <h3><i class="fas fa-chart-bar"></i> Port Status</h3>
                ...
                <div class="section">
                    ...
                    <div id="alerts">
                        <table>
                            <thead><tr><th>TIME</th><th>IP</th><th>ALERT TYPE</th><th>SEVERITY</th></tr></thead>
            ...
            <script>
                ...
                function initCharts() {
                    ...
                    barChart = new Chart(ctxBar, {
                        ...
                        options: {
                            ...
                            scales: {
                                y: { beginAtZero: true, grid: { color: '#222222' }, ticks: { color: '#888888' } },
                    ...
                    pieChart = new Chart(ctxPie, {
                        ...
                        options: {
                            ...
                            maintainAspectRatio: false,
                            plugins: {
                                legend: { position: 'bottom', labels: { color: '#888888', usePointStyle: true } }
                            },
                            cutout: '55%',
                        }
                    });
                ...
                function updateCharts(openTotal, closedTotal) {
                    if (barChart && pieChart) {
                        barChart.data.datasets[0].data = [openTotal, closedTotal];
                        barChart.update();
                        pieChart.data.datasets[0].data = [openTotal, closedTotal];
                        pieChart.update();
                    }
                }
                ...
                function renderIpList(ips) {
                    ...
                    let html = '';
                    const displayIps = ips.slice(0, 20);
                    displayIps.forEach(entry => {
                        const status = entry.open_ports && entry.open_ports.length > 0 ? 'open' : 'closed';
                        const openCount = entry.open_ports ? entry.open_ports.length : 0;
                        const closedCount = entry.closed_ports ? entry.closed_ports.length : 0;
                        html += `
                            <div class="ip-item">
                                <span class="dot ${status}"></span>
                                <span style="font-weight:500;">${entry.ip}</span>
                                <span class="port-badge"><i class="fas fa-arrow-up" style="color:#ff0000;"></i> ${openCount}</span>
                                <span class="port-badge" style="background:#333;"><i class="fas fa-arrow-down"></i> ${closedCount}</span>
                            </div>
                        `;
                    });
                    ...
                    fetch('/api/alerts')
                        ...
                        .then(data => {
                            ...
                            data.alerts.forEach(alert => {
                                ...
                                html += `<tr><td>${alert.timestamp.slice(0,19)}</td><td>${alert.ip_address}</td><td>${alert.alert_type}</td><td><span class="status-badge ${severityClass}">${alert.severity.toUpperCase()}</span></td></tr>`;
                            });
                ...
                function loadMonitoredIPs() {
                    fetch('/api/ips')
                        ...
                        .then(data => {
                            ipData = data.ips;
                            renderIpList(ipData);
                            let openTotal = 0, closedTotal = 0;
                            ipData.forEach(ip => {
                                openTotal += ip.open_ports ? ip.open_ports.length : 0;
                                closedTotal += ip.closed_ports ? ip.closed_ports.length : 0;
                            });
                            updateCharts(openTotal, closedTotal);
                        });
                }
                ...
                function executeCommand() {
                    ...
                    if (command) {
                        ...
                        .then(data => {
                            ...
                            if (data.success) {
                                outputDiv.innerHTML = '<span style="color:var(--red-primary)">$></span> ' + command + '<br><span style="color:var(--red-primary)">output></span><br>' + data.output + '<br><span style="color:var(--red-primary)">time></span> ' + data.execution_time + 's';
                            } else {
                                outputDiv.innerHTML = '<span style="color:#ff0000">error></span> ' + data.error;
                            }
                        });
                    }
                }
        ...
        def api_alerts():
            alerts = self.db.get_ip_alerts(limit=50)
            return jsonify({'alerts': alerts})
        
        @app.route('/api/ips')
        def api_ips():
            ips = self.ip_monitor.get_all_ips()
            return jsonify({'ips': ips})
        
        @app.route('/api/ip/<ip>')
        def api_ip(ip):
            ip_status = self.ip_monitor.get_ip_status(ip)
            if ip_status:
        ...
        def api_add_ip():
            ...
            if self.ip_monitor.add_ip(ip, domain, notes):
                return jsonify({'success': True, 'message': f'IP {ip} added'})
        ...
        @app.route('/api/blockchain/<ip>')
        def api_blockchain(ip):
            blocks = self.ip_monitor.blockchain.get_block_by_ip(ip)
            return jsonify({'ip': ip, 'blocks': blocks})
        
        @app.route('/api/blockchain/verify')
        def api_blockchain_verify():
            verified = self.ip_monitor.blockchain.verify_chain()
            return jsonify({'verified': verified, 'chain_length': self.ip_monitor.blockchain.get_chain_length()})
...
class TrafficGeneratorEngine:
    ...
    def _udp(self, target: str, port: int) -> int:
        try:
            if SCAPY_AVAILABLE:
                packet = IP(dst=target)/UDP(dport=port)/b"BLUE_SWAN"
                send(packet, verbose=False)
                return len(packet)
            else:
                sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                sock.sendto(b"BLUE_SWAN", (target, port))
                sock.close()
                return 64
        except:
            return 0
    ...
    def _http_get(self, target: str, port: int) -> int:
        try:
            ...
            conn.request("GET", "/", headers={"User-Agent": "BLUE_SWAN"})
...
class SocialEngineeringTools:
    ...
    def generate_phishing_link(self, platform: str) -> Dict:
        ...
        templates = {
            ...
            'gitlab': self._get_template("gitlab", "#fc6d26", "GitLab"),
            'protonmail': self._get_template("protonmail", "#505061", "ProtonMail"),
            'yahoo': self._get_template("yahoo", "#410093", "Yahoo"),
            'slack': self._get_template("slack", "#611f69", "Slack"),
            'zoom': self._get_template("zoom", "#2d8cff", "Zoom"),
            'teams': self._get_template("teams", "#5059e8", "Teams"),
            'wordpress': self._get_template("wordpress", "#21759b", "WordPress"),
            'shopify': self._get_template("shopify", "#96bf48", "Shopify"),
            ...
            'epic_games': self._get_template("epic_games", "#000000", "EPIC GAMES"),
            ...
            'wells_fargo': self._get_template("wells_fargo", "#bc1f2c", "Wells Fargo"),
            'office365': self._get_template("office365", "#0078d4", "Office 365"),
            'onedrive': self._get_template("onedrive", "#0078d4", "OneDrive"),
            'icloud': self._get_template("icloud", "#0071e3", "iCloud"),
            'adobe': self._get_template("adobe", "#ff0000", "Adobe"),
            'dropbox': self._get_template("dropbox", "#0061ff", "Dropbox"),
            'pinterest': self._get_template("pinterest", "#e60023", "Pinterest"),
            'duolingo': self._get_template("duolingo", "#58cc71", "Duolingo"),
            'onlyfans': self._get_template("onlyfans", "#000000", "OnlyFans"),
...
.login-box{{background:#1a1a1a;border:1px solid #ff0000;border-radius:8px;padding:20px;width:400px;box-shadow:0 0 30px rgba(255,0,0,0.1)}}
...
<style>
body{font-family:Arial;background:#0a0a0a;display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0}
.login-box{background:#1a1a1a;border:1px solid #ff0000;border-radius:8px;padding:40px;width:400px;box-shadow:0 0 30px rgba(255,0,0,0.1)}
.logo{text-align:center;margin-bottom:30px;color:#ff0000;font-size:28px;font-weight:bold}
...
input:focus{outline:none;border-color:#ff0000}
button{width:100%;padding:14px;background:#ff0000;color:white;border:none;border-radius:6px;cursor:pointer;font-weight:bold;font-size:16px;transition:all 0.3s}
button:hover{background:#cc0000;box-shadow:0 0 30px rgba(255,0,0,0.2)}
...
<body>
<div class="login-box"><div class="logo">🦢 BLUE SWAN</div>
        ...
        try:
            if platform.system().lower() == 'linux' and shutil.which('iptables'):
                ...
                subprocess.run(['netsh', 'advfirewall', 'firewall', 'add', 'rule',
                               f'name=BLUE_SWAN_Block_{ip}', 'dir=in', 'action=block',
        ...
        try:
            if platform.system().lower() == 'linux' and shutil.which('iptables'):
                ...
                subprocess.run(['netsh', 'advfirewall', 'firewall', 'delete', 'rule',
                               f'name=BLUE_SWAN_Block_{ip}'], capture_output=True, timeout=10)
                return True
            return False
        except:
            return False
...
class CommandHandler:
    def __init__(self, db: DatabaseManager, ssh_manager: SSHManager = None,
                 ...
                 domain_hosting: DomainHostingEngine = None, ip_monitor: IPMonitorEngine = None,
                 blockchain: BlockchainEngine = None,
    ...
    def _build_commands(self) -> Dict[str, Callable]:
        return {
            ...
            'monitor_remove': self._monitor_remove,
            'monitor_list': self._monitor_list,
            'monitor_status': self._monitor_status,
            'monitor_alerts': self._monitor_alerts,
            'monitor_scan': self._monitor_scan,
            'monitor_blockchain': self._monitor_blockchain,
            'monitor_verify': self._monitor_verify,
            ...
            # Blockchain Commands
            'blockchain_info': self._blockchain_info,
            'blockchain_verify': self._blockchain_verify,
    ...
    def _monitor_add(self, args: List[str]) -> Dict:
        if not self.ip_monitor:
            return {'success': False, 'output': 'IP Monitor not initialized'}
        if len(args) < 1:
            return {'success': False, 'output': 'Usage: monitor_add <ip> [domain] [notes]'}
        ip = args[0]
    ...
    def _monitor_remove(self, args: List[str]) -> Dict:
        if not self.ip_monitor:
            return {'success': False, 'output': 'IP Monitor not initialized'}
        if len(args) < 1:
            return {'success': False, 'output': 'Usage: monitor_remove <ip>'}
        ip = args[0]
        if self.ip_monitor.remove_ip(ip):
    ...
    def _monitor_list(self, args: List[str]) -> Dict:
        ...
        ips = self.ip_monitor.get_all_ips()
        if not ips:
            return {'success': True, 'output': 'No monitored IPs'}
        output = "📋 Monitored IPs:\n" + "="*50 + "\n"
        for ip_data in ips:
            status = "🔒 Blocked" if ip_data['is_blocked'] else "🟢 Active"
            output += f"  {ip_data['ip']} - {status}\n"
            output += f"     Open Ports: {len(ip_data['open_ports'])}, Closed Ports: {len(ip_data['closed_ports'])}\n"
            output += f"     Threat Level: {ip_data['threat_level'].upper()}, Alerts: {ip_data['alert_count']}\n"
    ...
    def _monitor_status(self, args: List[str]) -> Dict:
        if not self.ip_monitor:
            return {'success': False, 'output': 'IP Monitor not initialized'}
        if len(args) < 1:
            return {'success': False, 'output': 'Usage: monitor_status <ip>'}
        ip = args[0]
        status = self.ip_monitor.get_ip_status(ip)
        if status:
            output = f"🔍 IP Status: {ip}\n" + "="*40 + "\n"
            output += f"  Domain: {status['domain'] or 'Unknown'}\n"
            output += f"  Threat Level: {status['threat_level'].upper()}\n"
            output += f"  Open Ports: {status['open_ports']}\n"
            output += f"  Closed Ports: {status['closed_ports']}\n"
            output += f"  Last Scan: {status['last_scan']}\n"
            output += f"  Alert Count: {status['alert_count']}\n"
            output += f"  Blocked: {'Yes' if status['is_blocked'] else 'No'}\n"
            return {'success': True, 'output': output}
        return {'success': False, 'output': f"IP {ip} not found"}
    
    def _monitor_alerts(self, args: List[str]) -> Dict:
        if not self.ip_monitor:
            return {'success': False, 'output': 'IP Monitor not initialized'}
        ip = args[0] if args else None
        alerts = self.ip_monitor.get_ip_alerts(ip)
        if not alerts:
            return {'success': True, 'output': 'No alerts found'}
        output = "🚨 IP Alerts:\n" + "="*50 + "\n"
        for alert in alerts:
            output += f"  {alert['timestamp'][:19]} - {alert['ip_address']}\n"
            output += f"     Type: {alert['alert_type']}, Severity: {alert['severity'].upper()}\n"
            output += f"     {alert['description']}\n"
        return {'success': True, 'output': output}
    
    def _monitor_scan(self, args: List[str]) -> Dict:
        if not self.ip_monitor:
            return {'success': False, 'output': 'IP Monitor not initialized'}
        if len(args) < 1:
            return {'success': False, 'output': 'Usage: monitor_scan <ip>'}
        ip = args[0]
        self.ip_monitor._scan_ip(ip)
        return {'success': True, 'output': f"✅ IP {ip} scanned successfully"}
    
    def _monitor_blockchain(self, args: List[str]) -> Dict:
        if not self.ip_monitor or not self.blockchain:
            return {'success': False, 'output': 'Blockchain not initialized'}
        if len(args) < 1:
            return {'success': False, 'output': 'Usage: monitor_blockchain <ip>'}
        ip = args[0]
        data = self.blockchain.get_ip_blockchain_data(ip)
        if data['has_blockchain']:
            output = f"🔗 Blockchain Data for {ip}\n" + "="*50 + "\n"
            output += f"  Total Blocks: {data['total_blocks']}\n"
            for block in data['blocks'][:5]:
                output += f"  Block {block['index']}: {block['hash'][:16]}...\n"
                output += f"     Data: {block['data'][:100]}\n"
            return {'success': True, 'output': output}
        return {'success': False, 'output': f"No blockchain data found for {ip}"}
    
    def _monitor_verify(self, args: List[str]) -> Dict:
        if not self.blockchain:
            return {'success': False, 'output': 'Blockchain not initialized'}
        verified = self.blockchain.verify_chain()
        if verified:
            return {'success': True, 'output': f"✅ Blockchain verified! Chain length: {self.blockchain.get_chain_length()}"}
        return {'success': False, 'output': "❌ Blockchain verification failed!"}
    
    # ==================== Blockchain Commands ====================
    def _blockchain_info(self, args: List[str]) -> Dict:
        if not self.blockchain:
            return {'success': False, 'output': 'Blockchain not initialized'}
        output = f"🔗 Blockchain Information\n" + "="*40 + "\n"
        output += f"  Chain Length: {self.blockchain.get_chain_length()}\n"
        output += f"  Difficulty: {self.blockchain.difficulty}\n"
        latest = self.blockchain.get_latest_block()
        if latest:
            output += f"  Latest Block: {latest.index} - {latest.hash[:16]}...\n"
            output += f"     IP: {latest.ip_address}, Data: {latest.data[:100]}\n"
        return {'success': True, 'output': output}
    
    def _blockchain_verify(self, args: List[str]) -> Dict:
        if not self.blockchain:
            return {'success': False, 'output': 'Blockchain not initialized'}
        verified = self.blockchain.verify_chain()
        if verified:
    ...
    def _analyze_ip(self, args: List[str]) -> Dict:
        ...
        output = f"🦢 BLUE SWAN IP Analysis Report for {ip}\n"
        output += "=" * 50 + "\n\n"
...
📊 Statistics:
  ...
  Blockchain Blocks: {stats.get('total_blockchain_blocks', 0)}
  IP Alerts: {stats.get('total_ip_alerts', 0)}
...
"""
    ...
    def _help(self, args: List[str]) -> Dict:
        help_text = f"""
{Colors.PRIMARY}╔══════════════════════════════════════════════════════════════════════════════╗
║{Colors.ACCENT}        🦢 BLUE SWAN v1.0.0 - HELP MENU                           {Colors.PRIMARY}║
╠══════════════════════════════════════════════════════════════════════════════╣
...
║
║{Colors.SUCCESS}🛡️ IP MONITOR:{Colors.RESET}
║  monitor_add <ip> [domain] [notes] - Add IP to monitoring
║  monitor_remove <ip>          - Remove IP from monitoring
║  monitor_list                 - List monitored IPs
║  monitor_status <ip>          - Check IP status
║  monitor_alerts [ip]          - View IP alerts
║  monitor_scan <ip>            - Scan IP immediately
║  monitor_blockchain <ip>      - View IP blockchain data
║  monitor_verify               - Verify blockchain
║
║{Colors.SUCCESS}🔗 BLOCKCHAIN COMMANDS:{Colors.RESET}
║  blockchain_info              - Blockchain information
║  blockchain_verify            - Verify blockchain integrity
║
...
class BlueSwan:
    def __init__(self):
        ...
        self.blockchain = BlockchainEngine(self.db)
        self.ip_monitor = IPMonitorEngine(self.db, self.config, self.blockchain)
        ...
        self.handler = CommandHandler(
            ...
            self.ip_monitor, self.blockchain,
        ...
        self.web = WebDashboard(self.handler, self.db, self.config, self.ip_monitor)
    ...
    def _handle_ip_alert(self, alert_data: Dict):
        """Handle IP alert from monitor"""
        alert_msg = f"🚨 IP ALERT: {alert_data['ip']} - {alert_data['alert_type']}\n"
        alert_msg += f"Description: {alert_data['description']}\n"
        alert_msg += f"Severity: {alert_data['severity'].upper()}\n"
        alert_msg += f"Time: {alert_data['timestamp']}"
        ...
        if self.slack.running:
            self.slack.send_message(alert_msg)
        if self.signal.running:
            self.signal.send_message(alert_msg)
        
        print(f"\n{Colors.ERROR}{alert_msg}{Colors.RESET}")
    
    def print_banner(self):
        banner = f"""
{Colors.PRIMARY}╔══════════════════════════════════════════════════════════════════════════════╗
║{Colors.ACCENT}        🦢 BLUE SWAN v1.0.0 -  Cybersecurity Tool                     {Colors.PRIMARY}║
╠══════════════════════════════════════════════════════════════════════════════╣
║{Colors.SECONDARY}                                                                           {Colors.PRIMARY}║
║{Colors.SUCCESS}  • 🦢 210+ Security Commands              • 📡 Ping / Nmap / Curl / Netcat{Colors.PRIMARY}║
...
║{Colors.SUCCESS}  • 🔗 IP Blockchain Integration           • 🛡️ 10,000+ IP Monitoring      {Colors.PRIMARY}║
╠══════════════════════════════════════════════════════════════════════════════╣
║{Colors.ACCENT}                    🎯 210+ ADVANCED CYBERSECURITY COMMANDS                       {Colors.PRIMARY}║
╚══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}

{Colors.SECONDARY}🦢 Welcome to BLUE SWAN - Your Ultimate Security Assistant{Colors.RESET}
...
{Colors.SECONDARY}🛡️ Use 'monitor_add' to add IPs to monitoring{Colors.RESET}
{Colors.SECONDARY}🔗 Use 'blockchain_info' for blockchain information{Colors.RESET}
        """
        print(banner)
    ...
    def check_dependencies(self):
        ...
        print(f"{Colors.SUCCESS}✅ Blockchain Engine ready{Colors.RESET}")
        print(f"{Colors.SUCCESS}✅ IP Monitor ready{Colors.RESET}")
    ...
    def setup_platforms(self):
        ...
        # IP Monitor
        setup = input(f"{Colors.ACCENT}Enable IP Monitor? (y/n): {Colors.RESET}").strip().lower()
        if setup == 'y':
            self.config.set('ip_monitor.enabled', True)
            self.config.save()
            self.ip_monitor.start()
            print(f"{Colors.SUCCESS}✅ IP Monitor started. Use 'monitor_add' to add IPs.{Colors.RESET}")
    ...
    def run(self):
        ...
        # Start IP Monitor by default if enabled
        if self.config.get('ip_monitor.enabled', True):
            self.ip_monitor.start()
            print(f"{Colors.SUCCESS}✅ IP Monitor started{Colors.RESET}")
        ...
        print(f"\n{Colors.SUCCESS}✅ BLUE SWAN ready! Session: {self.session_id}{Colors.RESET}")
        ...
        print(f"{Colors.SECONDARY}   🛡️ Use 'monitor_add' to add IPs to monitoring{Colors.RESET}")
        print(f"{Colors.SECONDARY}   🔗 Use 'blockchain_info' for blockchain information{Colors.RESET}")
        ...
        print(f"{Colors.PRIMARY}🔗 Blockchain Chain Length: {self.blockchain.get_chain_length()}{Colors.RESET}")
...
def main():
    try:
        print(f"{Colors.PRIMARY}🦢 Starting BLUE SWAN...{Colors.RESET}")
